 ...
 */
class ScreenWP_Display {

	/**
	 * The ScreenWP Display post type name.
	 *
	 * @since    1.0.0
	 * @access   private
	 * @var      string    $post_type_name    The ScreenWP Display post type name.
	 */
	const post_type_name = 'screenwp_display';
	...
	public function add_reset_request() {
		update_post_meta( $this->ID, 'screenwp_reset_display', 1 );
	}
	...
	public function classes( $classes = array() ) {
		...
		$classes[] = 'screenwp-display';

		if ( $this->is_reset_requested() && empty( $_GET['screenwp-preview'] ) ) {
			// Reset is requested and we are not previewing, add class to invoke reset
			$classes[] = 'screenwp-reset-display';
	...
	public function delete_reset_request() {
		delete_post_meta( $this->ID, 'screenwp_reset_display' );
	}
	 ...
	 *
	 * @access	public
	 * @return	ScreenWP_Channel	The currently active channel for this display.
	 */
	 ...
	 *
	 * @access	public
	 * @return	ScreenWP_Channel	The default channel for this display.
	 */
	public function get_default_channel() {

		if ( ! isset( $this->default_channel ) ) {

			$default_channel = get_post_meta( $this->ID, ScreenWP_Channel::post_type_name, true );
	...
	public function get_schedule() {
		...
		$schedule = get_post_meta( $this->ID, 'screenwp_display_schedule', false );
	...
	private function is_reset_requested() {
		return (bool) get_post_meta( $this->ID, 'screenwp_reset_display', true );
	}
}
