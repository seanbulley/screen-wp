...
class VUWU_Slide {
	...
	/**
	 * The VUWU Slide post type name.
	 *
	 * @since    1.0.0
	 * @access   private
	 * @var      string    $post_type_name    The VUWU Slide post type name.
	 */
	const post_type_name = 'vuwu_slide';
	...
	public function background( $template_args = false ) {
		VUWU_Templates::get_template( 'slides/backgrounds/' . $this->get_background() . '.php', $template_args );
	}
	...
	public function background_classes( $classes = array() ) {
		...
		$classes[] = 'vuwu-slide-background';
		$classes[] = 'vuwu-slide-background-' . $this->get_background();
	...
	public function classes( $classes = array() ) {
		...
		$classes[] = 'vuwu-slide';
		$classes[] = 'vuwu-slide-' . $this->get_format();
		$classes[] = 'vuwu-slide-background-' . $this->get_background();

		if ( VUWU_Channel::post_type_name == get_post_type( get_queried_object_id() ) ) {
			$channel = new VUWU_Channel( get_queried_object_id() );
		}

		if ( VUWU_Display::post_type_name == get_post_type( get_queried_object_id() ) ) {
			$display = new VUWU_Display( get_queried_object_id() );
	...
	public function default_background( $template_args = false ) {
		if ( VUWU_Slides::slide_format_has_default_background_template( self::get_format() ) ) {
			VUWU_Templates::get_template( 'slides/backgrounds/default-' . $this->get_format() . '.php', $template_args );
		}
	}
	...
	public function data_attr( $data = array() ) {
		...
		if (!empty ($channel) )	{
			$data['vuwu-slide-duration'] = $channel->get_slides_duration();
		}
	...
	public function get_background() {
		...
		$slide_background_keys = array_keys( VUWU_Slides::get_slide_backgrounds() );
	...
	public function get_format() {
		...
		$slide_format_keys = array_keys( VUWU_Slides::get_slide_formats() );
	...
	public function get_image_url() {
		_deprecated_function( 'VUWU_Slide::get_image_url()', '1.4.0', '' );

		$attachment_id = get_post_meta( $this->ID, 'slide_bg_image_image', true );
		$attachment_src = wp_get_attachment_image_src( $attachment_id, 'vuwu_fhd_square' );
	...
	public function image_url() {
		_deprecated_function( 'VUWU_Slide::image_url()', '1.4.0', '' );
	...
	public function is_stack() {
		return VUWU_Slides::slide_format_is_stack( self::get_format() );
	}
}
