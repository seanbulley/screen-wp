 ...
 */
class ScreenWP_Updater {

	/**
	 * Adds an action on init that flushes the rewrite rules.
	 *
	 * @since	1.5.4
	 * @since	1.5.6	Changed the callback from the non-existing method ScreenWP_Updater::flush_rewrite_rules()
	...
	static function get_db_version() {
		return get_option( 'screenwp_plugin_version' );
	}
	...
	static function update() {
	    ...
	    if ( $db_version === ScreenWP::get_version() ) {
		...
		if ( empty( $db_version ) ) {
			...
			$db_version = ScreenWP::get_version();
		}
		...
		self::update_db_version( ScreenWP::get_version() );
	...
	static function update_db_version( $version ) {
		update_option( 'screenwp_plugin_version', $version );
	}
	...
	static function update_to_1_4_0() {
		...
		$slides = ScreenWP_Slides::get_posts( $args );
