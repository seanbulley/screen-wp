...
class VUWU_Updater {
	 ...
	 *
	 * @since	1.5.4
	 * @since	1.5.6	Changed the callback from the non-existing method VUWU_Updater::flush_rewrite_rules()
	...
	static function get_db_version() {
		return get_option( 'vuwu_plugin_version' );
	}
	...
	static function update() {
	    ...
	    if ( $db_version === VUWU::get_version() ) {
		...
		self::update_db_version( VUWU::get_version() );
	...
	static function update_db_version( $version ) {
		update_option( 'vuwu_plugin_version', $version );
	}
	...
	static function update_to_1_4_0() {
		...
		$slides = VUWU_Slides::get_posts( $args );
