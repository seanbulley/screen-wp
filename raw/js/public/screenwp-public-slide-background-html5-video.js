var screenwp_slide_bg_html5_video_selector = '.screenwp-slide-background-html5-video';
...
jQuery(document).ready(function() {
	...
	screenwp_slide_bg_html5_video_bind_ticker_events();
});
 ...
 * @since	1.6.0
 */
function screenwp_slide_bg_html5_video_bind_ticker_events() {
	...
	jQuery('body').on('slides:before-binding-events', screenwp_slides_selector, function ( event ) {
		...
		jQuery('body').on('slides:next-slide', screenwp_slides_selector, function( event ) {
			...
			// Set container
			var $container = jQuery(screenwp_slide_bg_html5_video_selector).filter('.active').find('.html5-video-container');

			// Set video reference
			var vid = $container.find('video').get(0);

			if (vid && 1 == $container.data('screenwp-hold-slide')) {
				...
				if ( ( vid.currentTime > 0 && !vid.paused && !vid.ended && vid.readyState > 2 ) ) {
					...
					if ( screenwp_slide_bg_html5_video_is_almost_ended($container, vid) ) {
	...
	jQuery('body').on('slide:became-active', screenwp_slide_bg_html5_video_selector, function( event ) {
		...
		if (vid) {
			...
			vid.addEventListener('playing', function screenwp_slide_bg_html5_video_seek_to_start() {
				// Remove event listerner to avoid endless loop of triggered 'playing' events
				this.removeEventListener('playing', screenwp_slide_bg_html5_video_seek_to_start, false);
				this.currentTime = $container.data('screenwp-video-start');
			}, false);
	...
	jQuery('body').on('slide:left-active', screenwp_slide_bg_html5_video_selector, function( event ) {
 ...
 */
function screenwp_slide_bg_html5_video_is_almost_ended($container,vid) {
	...
	if (vid) {
		...
		if ( current_time >= end - screenwp_ticker_css_transition_duration ) {
			// Video almost ended
			return true;
		}
	}
