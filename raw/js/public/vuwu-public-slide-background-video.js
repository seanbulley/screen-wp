var vuwu_slide_bg_video_selector = '.vuwu-slide-background-video';
var vuwu_yt_players = {};
var vuwu_yt_api_ready = false;

/**
 * Sets up the YouTube Video slide background public functionality.
 *
 * Functionality was copied from vuwu-public-slide-video.js (since 1.2.0, removed in 1.4.0).
 *
 * @since	1.4.0
...
jQuery(document).ready(function() {
	...
	vuwu_slide_bg_video_load_youtube_api();
	vuwu_slide_bg_video_bind_display_loading_events();
	vuwu_slide_bg_video_bind_ticker_events();
});
 ...
 * @since	1.4.0
 */
function vuwu_slide_bg_video_bind_display_loading_events() {

	jQuery('body').on('channel:replaced-channel', vuwu_channel_selector, function ( event ) {
		if (vuwu_yt_api_ready) {
			vuwu_slide_bg_video_init_video_placeholders();
			vuwu_slide_bg_video_cleanup_youtube_players();
		}
		else {
			vuwu_slide_bg_video_load_youtube_api();
		}
	...
	jQuery('body').on('slides:loaded-new-slide-group', vuwu_slides_selector, function ( event ) {
		if (vuwu_yt_api_ready) {
			vuwu_slide_bg_video_init_video_placeholders();
		}
		else {
	...
	jQuery('body').on('slides:removed-old-slide-group', vuwu_slides_selector, function ( event ) {
		vuwu_slide_bg_video_cleanup_youtube_players();
	});
}
 ...
 */
function vuwu_slide_bg_video_bind_ticker_events() {
	...
	jQuery('body').on('slides:before-binding-events', vuwu_slides_selector, function ( event ) {
		...
		jQuery('body').on('slides:next-slide', vuwu_slides_selector, function( event ) {
			...
			// Set container
			var container = jQuery(vuwu_slide_bg_video_selector).filter('.active').find('.youtube-video-container');

			// Set player reference
			var player = window.vuwu_yt_players[container.attr('id')]

			if (1 == container.data('vuwu-hold-slide')) {
				...
				if (player && typeof player.playVideo === 'function') {
					...
					else if ( current_time >= end - vuwu_ticker_css_transition_duration ) {
	...
	jQuery('body').on('slide:became-active', vuwu_slide_bg_video_selector, function( event ) {
	...
	jQuery('body').on('slide:left-active', vuwu_slide_bg_video_selector, function( event ) {
		...
		if (player && typeof player.playVideo === 'function') {
			...
			setTimeout(function() {
				player.seekTo(container.data('vuwu-video-start'));
				player.pauseVideo();
 ...
 */
function vuwu_slide_bg_video_cleanup_youtube_players() {
	for (var player_id in window.vuwu_yt_players) {
		if (!jQuery('#' + player_id).length) {
			// Video is no longer present in the document, remove its player reference
			delete window.vuwu_yt_players[player_id];

			// Remove the resize event trigger for this player
			jQuery(window).off('resize', function() {
				vuwu_slide_bg_video_resize_youtube_to_cover(player_id);
			});
		}
	}
}
 ...
 */
function vuwu_slide_bg_video_init_video_placeholders() {
	...
	jQuery('div.youtube-video-container').each(function() {
		...
		var video_id = container.data('vuwu-video-id');

		if (player_id && video_id) {
			// Set up player and store its reference
			window.vuwu_yt_players[player_id] = new YT.Player(player_id, {
				width: '1920',
				height: '1080',
				videoId: video_id,
				playerVars: {
				...
				events: {
					'onReady': vuwu_slide_bg_video_prepare_player_for_playback(player_id),
				}
			});
		}
	});
 ...
 * @since	1.4.0
 */
function vuwu_slide_bg_video_load_youtube_api() {
 ...
 *					Muting of video is now optional, based on the vuwu-output-sound data attribute.
 ...
 */
function vuwu_slide_bg_video_prepare_player_for_playback(player_id) {
	...
	return function(event) {
		...
		// Set player reference
		var player = window.vuwu_yt_players[player_id];
		...
		if (
			jQuery(vuwu_slides_selector).length &&
			! jQuery('#' + player_id).parents(vuwu_slide_bg_video_selector).hasClass('active')
		) {
 ...
 * @param	string	player_id	The ID of the player
 */
function vuwu_slide_bg_video_resize_youtube_to_cover(player_id) {
