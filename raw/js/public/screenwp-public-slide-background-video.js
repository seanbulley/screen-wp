var screenwp_slide_bg_video_selector = '.screenwp-slide-background-video';
var screenwp_yt_players = {};
var screenwp_yt_api_ready = false;

/**
 * Sets up the YouTube Video slide background public functionality.
 *
 * Functionality was copied from screenwp-public-slide-video.js (since 1.2.0, removed in 1.4.0).
 *
 * @since	1.4.0
...
jQuery(document).ready(function() {
	...
	screenwp_slide_bg_video_load_youtube_api();
	screenwp_slide_bg_video_bind_display_loading_events();
	screenwp_slide_bg_video_bind_ticker_events();
});
 ...
 * @since	1.4.0
 */
function screenwp_slide_bg_video_bind_display_loading_events() {

	jQuery('body').on('channel:replaced-channel', screenwp_channel_selector, function ( event ) {
		if (screenwp_yt_api_ready) {
			screenwp_slide_bg_video_init_video_placeholders();
			screenwp_slide_bg_video_cleanup_youtube_players();
		}
		else {
			screenwp_slide_bg_video_load_youtube_api();
		}
	...
	jQuery('body').on('slides:loaded-new-slide-group', screenwp_slides_selector, function ( event ) {
		if (screenwp_yt_api_ready) {
			screenwp_slide_bg_video_init_video_placeholders();
		}
		else {
	...
	jQuery('body').on('slides:removed-old-slide-group', screenwp_slides_selector, function ( event ) {
		screenwp_slide_bg_video_cleanup_youtube_players();
	});
}
 ...
 */
function screenwp_slide_bg_video_bind_ticker_events() {

	jQuery('body').on('slides:before-binding-events', screenwp_slides_selector, function ( event ) {
		...
		jQuery('body').on('slides:next-slide', screenwp_slides_selector, function( event ) {
			...
			// Set container
			var container = jQuery(screenwp_slide_bg_video_selector).filter('.active').find('.youtube-video-container');

			// Set player reference
			var player = window.screenwp_yt_players[container.attr('id')]

			if (1 == container.data('screenwp-hold-slide')) {
				...
				if (player && typeof player.playVideo === 'function') {
					...
					var end = container.data('screenwp-video-end');
					...
					else if ( current_time >= end - screenwp_ticker_css_transition_duration ) {
	...
	jQuery('body').on('slide:became-active', screenwp_slide_bg_video_selector, function( event ) {
	...
	jQuery('body').on('slide:left-active', screenwp_slide_bg_video_selector, function( event ) {
		...
		if (player && typeof player.playVideo === 'function') {
			...
			setTimeout(function() {
				player.seekTo(container.data('screenwp-video-start'));
				player.pauseVideo();
			}, screenwp_ticker_css_transition_duration * 1000);
		}
	});
}
 ...
 */
function screenwp_slide_bg_video_cleanup_youtube_players() {
	for (var player_id in window.screenwp_yt_players) {
		if (!jQuery('#' + player_id).length) {
			// Video is no longer present in the document, remove its player reference
			delete window.screenwp_yt_players[player_id];

			// Remove the resize event trigger for this player
			jQuery(window).off('resize', function() {
				screenwp_slide_bg_video_resize_youtube_to_cover(player_id);
			});
		}
	}
}
 ...
 */
function screenwp_slide_bg_video_init_video_placeholders() {
	...
	jQuery('div.youtube-video-container').each(function() {
		...
		var video_id = container.data('screenwp-video-id');

		if (player_id && video_id) {
			// Set up player and store its reference
			window.screenwp_yt_players[player_id] = new YT.Player(player_id, {
				width: '1920',
				height: '1080',
				videoId: video_id,
				playerVars: {
				...
				events: {
					'onReady': screenwp_slide_bg_video_prepare_player_for_playback(player_id),
				}
			});
		}
	});
 ...
 * @since	1.4.0
 */
function screenwp_slide_bg_video_load_youtube_api() {
 ...
 *					Muting of video is now optional, based on the screenwp-output-sound data attribute.
 ...
 */
function screenwp_slide_bg_video_prepare_player_for_playback(player_id) {
	...
	return function(event) {
		...
		// Set player reference
		var player = window.screenwp_yt_players[player_id];
		...
		if (!container.data('screenwp-output-sound')) {
		...
		if (
			jQuery(screenwp_slides_selector).length &&
			! jQuery('#' + player_id).parents(screenwp_slide_bg_video_selector).hasClass('active')
		) {
 ...
 * @param	string	player_id	The ID of the player
 */
function screenwp_slide_bg_video_resize_youtube_to_cover(player_id) {
