var screenwp_yt_player;

/**
 * Sets up the Video slide format admin functionality.
 *
 * Functionality was copied from screenwp-admin-slide-video.js (since 1.2.0, removed in 1.4.0).
 *
 * @since	1.4.0
...
jQuery( function() {
	...
	if (jQuery('#slide_bg_video_video_url').val() && jQuery('#slide_bg_video_video_url').val().length) {
		...
		screenwp_admin_slide_bg_video_validate_youtube_video_url();
	}
	...
	jQuery('#slide_bg_video_video_start').on('change', function() {
		...
		screenwp_admin_slide_bg_video_update_youtube_video_preview();
	});
	...
	jQuery('#slide_bg_video_enable_sound').on('change', function() {
		...
		screenwp_admin_slide_bg_video_update_player_mute();
	});
 ...
 * @since	1.4.0
 */
function screenwp_admin_load_youtube_api() {
 ...
 */
function screenwp_admin_slide_bg_video_prepare_player_for_playback() {
	if (window.screenwp_yt_player) {
		var player = window.screenwp_yt_player;
		screenwp_admin_slide_bg_video_update_player_mute();
		player.playVideo();
	}
}
 ...
 * @since	1.5.1
 */
function screenwp_admin_slide_bg_video_update_player_mute() {
	if (window.screenwp_yt_player) {
		var player = window.screenwp_yt_player;
 ...
 * @since	1.5.1	Updates the player's mute status before playing.
 */
function screenwp_admin_slide_bg_video_update_youtube_video_preview() {
	if (jQuery('#slide_bg_video_video_id').val() && jQuery('#slide_bg_video_video_id').val().length) {
		...
		if (window.screenwp_yt_player) {
			...
			screenwp_admin_load_youtube_api();
		}
	}
}
 ...
 * @since	1.4.0
 */
function screenwp_admin_slide_bg_video_validate_youtube_video_url() {
	var video_metadata = screenwp_get_video_id(jQuery('#slide_bg_video_video_url').val());
...
function onYouTubeIframeAPIReady() {
	...
	window.screenwp_yt_player = new YT.Player('screenwp-admin-video-preview', {
		width: '480',
		height: '270',
		videoId: video_id,
		playerVars: {
		...
		events: {
			'onReady': screenwp_admin_slide_bg_video_prepare_player_for_playback,
		}
	});
}