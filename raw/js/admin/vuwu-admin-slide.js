...
function init_slide_background_select() {
	...
	$slide_background_select = jQuery('#vuwu_slide_content select[name=slide_background]');
	slide_background = jQuery('#vuwu_slide_content select[name=slide_background]').val();
...
function update_slide_background_meta_boxes() {
	...
	$meta_boxes = jQuery('.vuwu_slide_backgrounds > *');
	slide_background = jQuery('#vuwu_slide_content select[name=slide_background]').val();

	$meta_boxes.hide().filter('#vuwu_slide_background_'+slide_background).show();
}
...
function update_slide_background_select() {
	...
	$slide_background_select = jQuery('#vuwu_slide_content select[name=slide_background]');
	slide_format = jQuery('#vuwu_slide_content select[name=slide_format]').val();
	slide_format_backgrounds = vuwu_slide_formats_backgrounds[slide_format];
...
function update_slide_format_meta_boxes() {
	...
	$meta_boxes = jQuery('.vuwu_slide_formats > *');
	slide_format = jQuery('#vuwu_slide_content select[name=slide_format]').val();

	$meta_boxes.hide().filter('#vuwu_slide_format_'+slide_format).show();
}

jQuery( function() {

	if (jQuery('#vuwu_slide_content select[name=slide_format], #vuwu_slide_content select[name=slide_background]').length) {
	...
	jQuery('#vuwu_slide_content select[name=slide_format]').on('change', function() {
		update_slide_background_select();
		update_slide_format_meta_boxes();
		update_slide_background_meta_boxes();
	});
	jQuery('#vuwu_slide_content select[name=slide_background]').on('change', function() {
		update_slide_background_meta_boxes();
	});
	...
	if (wp.media) {
		...
		jQuery('.slide_file_upload_button').on('click', function(event) {
			...
			file_frame = wp.media.frames.file_frame = wp.media({
				title: vuwu_slide_file_defaults[file_type].text_select,
				button: {
					text: vuwu_slide_file_defaults[file_type].text_use
				},
				library: {
					type: file_type
				},
