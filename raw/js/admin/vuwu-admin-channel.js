(function( $ ) {
	...
	function setup_slides_editor_actions() {
		...
		var $slides_editor = $('.vuwu_slides_editor');
		var $slides_editor_slides = $('.vuwu_slides_editor_slides');

		/* Set up add action in the slides editor */

		$('.vuwu_slides_editor_add_select').unbind('change').change(function(e) {
			...
			if ( slide_id > 0 ) {
				...
				var data = {
					'action': 'vuwu_slides_editor_add_slide',
					'channel_id': $slides_editor.data('channel-id'),
					'slide_id': slide_id,
					'nonce': vuwu_slides_editor_security.nonce,
				};

				$.post(ajaxurl, data, function(response) {
					if (response != '') {
						$slides_editor_slides.replaceWith(response);
						$('.vuwu_slides_editor_add_select').val('');
						setup_slides_editor_actions();
					}
				});
		...
		$('.vuwu_slides_editor_slides_slide_remove').unbind('click').click(function(e) {

			if (confirm(vuwu_slides_editor_defaults.confirm_remove_message)) {

				var data = {
					'action': 'vuwu_slides_editor_remove_slide',
					'channel_id': $slides_editor.data('channel-id'),
					'slide_key': $(e.currentTarget).parents('.vuwu_slides_editor_slides_slide').data('slide-key'),
					'nonce': vuwu_slides_editor_security.nonce,
				};
		...
		$slides_editor_slides.sortable({
			...
			update: function( event, ui ) {
				...
				var data = {
					'action': 'vuwu_slides_editor_reorder_slides', // Tell WordPress how to handle this ajax request
