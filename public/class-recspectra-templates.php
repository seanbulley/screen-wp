...
class Recspectra_Templates {
	...
	static function get_template( $template_name, $template_args = array(), $template_path = '', $default_path = '' ) {
		...
		if ( ! file_exists( $template_file ) ) {
			_doing_it_wrong( __FUNCTION__, sprintf( '<code>%s</code> does not exist.', $template_file ), RECSPECTRA_PLUGIN_VERSION );
			return false;
		}
	 ...
	 *
	 * Add-ons can add their plugin template path using Recspectra_Templates::register_plugin_template_path().
	 *
	 * @since	1.7.2
	 *
	...
	static function get_plugin_template_paths() {
		 ...
		 */
		$plugin_template_paths = apply_filters( 'recspectra/templates/plugin_template_paths', $plugin_template_paths );

		return $plugin_template_paths;
	}

	/**
	 * Locates a Recspectra template.
	 *
	 * Search Order:
	 * 1. /themes/theme/recspectra/$template_name
	 * 2. <registered plugin template paths>/$template_name.
	 * 3. /plugins/recspectra/public/templates/$template_name.
	 *
	 * Inspired by https://jeroensormani.com/how-to-add-template-files-in-your-plugin/
	 *
	 * @since	1.0.0
	...
	static function locate_template( $template_name, $template_path = '', $default_path = '' ) {
		...
		// Set template path to recspectra folder of theme / registered plugin.
		if ( ! $template_path ) {
			$template_path = 'recspectra/';
		}
		// Set default path to templates folder of Recspectra plugin.
		if ( ! $default_path ) {
		...
		// 3. Fall back to template file in Recspectra plugin (if no template found in previous step).
		if ( ! $template ) {
			$template = $default_path . $template_name;
		}

		return apply_filters( 'recspectra/templates/template', $template, $template_name, $template_path, $default_path );
	}
	...
	static function register_plugin_template_path( $template_path ) {
		add_filter(
			'recspectra/templates/plugin_template_paths',
	 ...
	 * Template loader.
	 *
	 * The template loader will check if WP is loading a template for a Recspectra post type
	...
	static function template_include( $template ) {
		...
		else if ( is_singular( Recspectra_Slide::post_type_name ) ) {
			$file = 'single-slide.php';
		}
		else if ( is_singular( Recspectra_Channel::post_type_name ) ) {
			$file = 'single-channel.php';
		}
		else if ( is_singular( Recspectra_Display::post_type_name ) ) {
