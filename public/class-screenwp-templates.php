...
class ScreenWP_Templates {
	...
	static function get_template( $template_name, $template_args = array(), $template_path = '', $default_path = '' ) {
		...
		if ( ! file_exists( $template_file ) ) {
			_doing_it_wrong( __FUNCTION__, sprintf( '<code>%s</code> does not exist.', $template_file ), SCREENWP_PLUGIN_VERSION );
			return false;
		}
	 ...
	 *
	 * Add-ons can add their plugin template path using ScreenWP_Templates::register_plugin_template_path().
	 *
	 * @since	1.7.2
	 *
	...
	static function get_plugin_template_paths() {
		 ...
		 */
		$plugin_template_paths = apply_filters( 'screenwp/templates/plugin_template_paths', $plugin_template_paths );

		return $plugin_template_paths;
	}

	/**
	 * Locates a ScreenWP template.
	 *
	 * Search Order:
	 * 1. /themes/theme/screenwp/$template_name
	 * 2. <registered plugin template paths>/$template_name.
	 * 3. /plugins/screenwp/public/templates/$template_name.
	 *
	 * Inspired by https://jeroensormani.com/how-to-add-template-files-in-your-plugin/
	 *
	 * @since	1.0.0
	...
	static function locate_template( $template_name, $template_path = '', $default_path = '' ) {
		...
		// Set template path to screenwp folder of theme / registered plugin.
		if ( ! $template_path ) {
			$template_path = 'screenwp/';
		}
		// Set default path to templates folder of ScreenWP plugin.
		if ( ! $default_path ) {
		...
		// 3. Fall back to template file in ScreenWP plugin (if no template found in previous step).
		if ( ! $template ) {
			$template = $default_path . $template_name;
		}

		return apply_filters( 'screenwp/templates/template', $template, $template_name, $template_path, $default_path );
	}
	...
	static function register_plugin_template_path( $template_path ) {
		add_filter(
			'screenwp/templates/plugin_template_paths',
	 ...
	 * Template loader.
	 *
	 * The template loader will check if WP is loading a template for a ScreenWP post type
	...
	static function template_include( $template ) {
		...
		if (
			is_singular( array( ScreenWP_Slide::post_type_name, ScreenWP_Channel::post_type_name, ScreenWP_Display::post_type_name ) ) &&
			is_user_logged_in( ) &&
			empty( $_GET['screenwp-preview'] )
		) {
			// Show inside preview iframe when logged in.
			$file = 'preview.php';
		}
		else if ( is_singular( ScreenWP_Slide::post_type_name ) ) {
			$file = 'single-slide.php';
		}
		else if ( is_singular( ScreenWP_Channel::post_type_name ) ) {
			$file = 'single-channel.php';
		}
		else if ( is_singular( ScreenWP_Display::post_type_name ) ) {
