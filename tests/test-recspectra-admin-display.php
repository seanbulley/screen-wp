...
class Test_Recspectra_Admin_Display extends Recspectra_UnitTestCase {

	function get_meta_boxes_for_display( $display_id ) {
		$this->assume_role( 'author' );
		set_current_screen( Recspectra_Display::post_type_name );

		do_action( 'add_meta_boxes', Recspectra_Display::post_type_name );
		ob_start();
		do_meta_boxes( Recspectra_Display::post_type_name, 'normal', get_post( $display_id ) );
	...
	function test_channel_editor_meta_box_is_displayed_on_display_admin_page() {
		...
		$this->assertContains( '<div id="recspectra_channel_editor" class="postbox', $meta_boxes );
	}
	...
	function test_channel_scheduler_meta_box_is_displayed_on_display_admin_page() {
		...
		$this->assertContains( '<div id="recspectra_channel_scheduler" class="postbox', $meta_boxes );
	}
	...
	function test_is_default_channel_column_empty_when_no_default_channel() {
		...
		ob_start();
		Recspectra_Admin_Display::do_channel_columns( 'default_channel', $display_id );
		$actual = ob_get_clean();
	...
	function test_is_active_channel_column_empty_when_no_default_channel() {
		...
		ob_start();
		Recspectra_Admin_Display::do_channel_columns( 'active_channel', $display_id );
		$actual = ob_get_clean();
	...
	function test_default_channel_html_contains_all_channels() {
		...
		$actual = Recspectra_Admin_Display::get_default_channel_html( get_post( $this->display1 ) );
	...
	function test_scheduled_channel_html_contains_all_channels() {
		...
		$actual = Recspectra_Admin_Display::get_scheduled_channel_html( get_post( $this->display1 ) );
