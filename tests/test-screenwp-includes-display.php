...
class Test_ScreenWP_Display extends ScreenWP_UnitTestCase {
	...
	function test_is_default_channel_used_when_schedule_has_no_channel() {
		...
		$display = new ScreenWP_Display( $display_id );
	...
	function test_is_default_channel_used_when_schedule_has_not_published_channel() {
		...
		$display_id = $this->factory->post->create( $display_args );
		add_post_meta( $display_id, ScreenWP_Channel::post_type_name, $channel_1_id );
	...
	function test_is_scheduled_channel_used_when_schedule_is_now() {
		...
		$_POST['screenwp_channel_editor_' . ScreenWP_Display::post_type_name] = $display_id;
		...
		ScreenWP_Admin_Display::save_display( $display_id );
	...
	function test_is_reset_request_added() {
		...
		$this->assertEmpty( get_post_meta( $display_id, 'screenwp_reset_display', true ) );

		$display = new ScreenWP_Display( $display_id );
		$display->add_reset_request();

		/* Check that reset request was added */
		$this->assertNotEmpty( get_post_meta( $display_id, 'screenwp_reset_display', true ) );
	}
	 ...
	 * @since	1.4.0
	 */
	function test_is_screenwp_reset_display_class_added_when_reset_is_requested() {
		...
		/* Check that no screenwp-reset-display class is present by default */
		ob_start();
		$display->classes();
		$actual = ob_get_clean();

		$expected = 'screenwp-reset-display';
		$this->assertNotContains( $expected, $actual );

		$display->add_reset_request();

		/* Check that screenwp-reset-display class is added */
		ob_start();
		$display->classes();
		$actual = ob_get_clean();
	 ...
	 * @since	1.4.0
	 */
	function test_is_screenwp_reset_display_class_not_added_when_reset_is_requested_and_previewing() {

		// We are previewing
		$_GET['screenwp-preview'] = 1;
		...
		/* Check that screenwp-reset-display class is not added */
		ob_start();
		$display->classes();
		$actual = ob_get_clean();
