...
class Test_Recspectra_Admin_Channel extends Recspectra_UnitTestCase {

	function get_meta_boxes_for_channel( $channel_id ) {
		$this->assume_role( 'author' );
		set_current_screen( Recspectra_Channel::post_type_name );

		do_action( 'add_meta_boxes', Recspectra_Channel::post_type_name );
		ob_start();
		do_meta_boxes( Recspectra_Channel::post_type_name, 'normal', get_post( $channel_id ) );
	...
	function test_does_channel_have_slides() {
		...
		$channel = new Recspectra_Channel( $this->channel1 );
	...
	function test_slides_editor_is_displayed_on_channel_admin_page() {
		...
		$this->assertContains( '<div class="recspectra_meta_box recspectra_slides_editor"', $meta_boxes );
	}

	function test_add_slide_html_is_displayed_on_channel_admin_page() {

		$add_slide_html = Recspectra_Admin_Channel::get_add_slide_html();
	...
	function test_slides_list_html_is_displayed_on_channel_admin_page() {
		...
		$slides_list_html = Recspectra_Admin_Channel::get_slides_list_html( get_post( $this->channel1 ) );
	...
	function test_are_slideshow_settings_saved() {
		...
		$_POST[ Recspectra_Channel::post_type_name.'_nonce' ] = wp_create_nonce( Recspectra_Channel::post_type_name );
		$_POST['recspectra_slides_settings_duration'] = $duration;
		$_POST['recspectra_slides_settings_transition'] = $transition;

		Recspectra_Admin_Channel::save_channel( $this->channel1 );

		$updated_channel = new Recspectra_Channel( $this->channel1 );
	...
	function test_add_slide_html_contains_all_slides() {
		...
		$actual = Recspectra_Admin_Channel::get_add_slide_html( get_post( $this->channel1 ) );
	...
	function test_is_stack_class_included_in_channel_admin_page_for_slide_stack() {
		...
		update_post_meta( $channel_id, Recspectra_Slide::post_type_name, array( $slide_stack_id ) );

		$meta_boxes = $this->get_meta_boxes_for_channel( $channel_id );

		$this->assertContains( '<div class="recspectra_slides_editor_slides_slide recspectra-slide-is-stack', $meta_boxes );
	}
	...
	function test_is_stack_class_not_included_in_channel_admin_page_for_single_slide() {
		...
		$channel_id = $this->factory->post->create( $channel_args );
		update_post_meta( $channel_id, Recspectra_Slide::post_type_name, array( $slide_single_id ) );

		$meta_boxes = $this->get_meta_boxes_for_channel( $channel_id );

		$this->assertContains( '<div class="recspectra_slides_editor_slides_slide', $meta_boxes );
		$this->assertNotContains( '<div class="recspectra_slides_editor_slides_slide recspectra-slide-is-stack', $meta_boxes );
	}
	...
	function test_is_overlay_with_info_about_slide_included_in_channel_admin_page() {
		...
		$channel_id = $this->factory->post->create( $channel_args );
		update_post_meta( $channel_id, Recspectra_Slide::post_type_name, array( $slide_id ) );

		$meta_boxes = $this->get_meta_boxes_for_channel( $channel_id );

		$this->assertContains( '<div class="recspectra_slides_editor_slides_slide_iframe_container_overlay', $meta_boxes );
 ...
 * @group ajax
 */
class Test_Recspectra_Admin_Channel_Ajax extends Recspectra_Ajax_UnitTestCase {

	function add_slide_to_channel( $slide_id, $channel_id ) {

		$this->_setRole( 'administrator' );

		$_POST['action'] = 'recspectra_slides_editor_add_slide';
		$_POST['channel_id'] = $channel_id;
		$_POST['slide_id'] = $slide_id;
		$_POST['nonce'] = wp_create_nonce( 'recspectra_slides_editor_ajax_nonce' );

		try {
			$this->_handleAjax( 'recspectra_slides_editor_add_slide' );
		}
	...
	function remove_slide_from_channel( $slide_key, $channel_id ) {
		...
		$_POST['action'] = 'recspectra_slides_editor_remove_slide';
		$_POST['channel_id'] = $channel_id;
		$_POST['slide_key'] = $slide_key;
		$_POST['nonce'] = wp_create_nonce( 'recspectra_slides_editor_ajax_nonce' );

		try {
			$this->_handleAjax( 'recspectra_slides_editor_remove_slide' );
		}
		catch ( WPAjaxDieContinueException $e ) {
			// We expected this, do nothing.
		}
