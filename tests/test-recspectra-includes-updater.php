...
class Test_Recspectra_Updater extends Recspectra_UnitTestCase {
	...
	function test_is_database_version_updated_after_plugin_update() {
		...
		Recspectra_Updater::update_db_version( '1.0.0' );

		$actual = Recspectra_Updater::update();
		$expected = true;
		$this->assertEquals( $expected, $actual );

		$actual = Recspectra_Updater::get_db_version();
		$expected = Recspectra::get_version();
		$this->assertEquals( $expected, $actual );
	}
	...
	function test_is_database_version_updated_after_plugin_update_with_no_database_version_set() {
		...
		delete_option( 'recspectra_plugin_version' );
	...
	function test_is_database_update_skipped_when_database_is_up_to_date() {
		...
		Recspectra_Updater::update_db_version( Recspectra::get_version() );
	...
	function test_are_slides_correctly_converted_on_update_to_1_4_0() {
		...
		// Run update to 1.4.0
		Recspectra_Updater::update_to_1_4_0();
	...
	function test_are_displays_reset_when_updating_from_1_0_0() {
		...
		$this->assertEmpty( get_post_meta( $this->display1, 'recspectra_reset_display', true ) );

		Recspectra_Updater::update();

		/* Check that reset request was added */
		$this->assertNotEmpty( get_post_meta( $this->display1, 'recspectra_reset_display', true ) );
	}
