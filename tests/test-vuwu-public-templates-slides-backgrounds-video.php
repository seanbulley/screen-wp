...
class Test_VUWU_Public_Templates_Slides_Backgrounds_Video extends VUWU_UnitTestCase {
	...
	function test_are_all_slide_background_video_properties_included_in_slide() {
		...
		$expected = 'data-vuwu-video-id="' . $video_id . '"';
		$this->assertContains( $expected, $actual );

		$expected = 'data-vuwu-video-start="' . $video_start . '"';
		$this->assertContains( $expected, $actual );

		$expected = 'data-vuwu-video-end="' . $video_end . '"';
		$this->assertContains( $expected, $actual );

		$expected = 'data-vuwu-hold-slide="' . $hold_slide . '"';
		$this->assertContains( $expected, $actual );

		$expected = 'data-vuwu-output-sound="' . $enable_sound . '"';
		$this->assertContains( $expected, $actual );
	}
}
