...
class Test_VUWU_Display extends VUWU_UnitTestCase {
	...
	function test_is_default_channel_used_when_schedule_has_no_channel() {
		...
		update_post_meta( $display_id, 'vuwu_display_schedule', $schedule );

		$display = new VUWU_Display( $display_id );
	...
	function test_is_default_channel_used_when_schedule_has_not_published_channel() {
		...
		$display_id = $this->factory->post->create( $display_args );
		add_post_meta( $display_id, VUWU_Channel::post_type_name, $channel_1_id );
	...
	function test_is_default_channel_not_returned_when_channel_is_not_published() {
        ...
        function test_highest_priority_channel_is_returned() {
                ...
                $_POST['vuwu_channel_editor_' . VUWU_Display::post_type_name] = $display_id;
                $_POST['vuwu_channel_editor_default_channel'] = $low_priority_channel;
                ...
                $_POST['vuwu_channel_schedule'] = $schedule;

                VUWU_Admin_Display::save_display( $display_id );
	...
	function test_is_reset_request_added() {
		...
		$this->assertEmpty( get_post_meta( $display_id, 'vuwu_reset_display', true ) );

		$display = new VUWU_Display( $display_id );
		$display->add_reset_request();

		/* Check that reset request was added */
		$this->assertNotEmpty( get_post_meta( $display_id, 'vuwu_reset_display', true ) );
	}
	 ...
	 * @since	1.4.0
	 */
	function test_is_vuwu_reset_display_class_added_when_reset_is_requested() {
		...
		/* Check that no vuwu-reset-display class is present by default */
		ob_start();
		$display->classes();
		$actual = ob_get_clean();

		$expected = 'vuwu-reset-display';
		$this->assertNotContains( $expected, $actual );

		$display->add_reset_request();

		/* Check that vuwu-reset-display class is added */
		ob_start();
		$display->classes();
		$actual = ob_get_clean();
	 ...
	 * @since	1.4.0
	 */
	function test_is_vuwu_reset_display_class_not_added_when_reset_is_requested_and_previewing() {
		...
		/* Check that vuwu-reset-display class is not added */
		ob_start();
		$display->classes();
		$actual = ob_get_clean();
