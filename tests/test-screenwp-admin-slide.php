...
class Test_ScreenWP_Admin_Slide extends ScreenWP_UnitTestCase {

	function get_meta_boxes_for_slide( $slide_id ) {
		$this->assume_role( 'author' );
		set_current_screen( ScreenWP_Slide::post_type_name );

		do_action( 'add_meta_boxes', ScreenWP_Slide::post_type_name );
		ob_start();
		do_meta_boxes( ScreenWP_Slide::post_type_name, 'normal', get_post( $slide_id ) );
	...
	function test_is_slide_format_default_saved() {
		...
		$updated_slide = new ScreenWP_Slide( $this->slide1 );
	...
	function test_slide_format_select_is_displayed_on_slide_admin_page() {
		...
		$this->assertContains( '<div class="screenwp_slide_select_format"', $meta_boxes );
	}
	...
	function test_slide_background_select_is_displayed_on_slide_admin_page() {
		...
		$this->assertContains( '<div class="screenwp_slide_select_background"', $meta_boxes );
	}
	...
	function test_all_slide_format_options_are_displayed_on_slide_admin_page() {
		...
		$slide_formats = ScreenWP_Slides::get_slide_formats();
	...
	function test_all_slide_background_options_are_displayed_on_slide_admin_page() {
		...
		$slide_backgrounds = ScreenWP_Slides::get_slide_backgrounds();
	...
	function test_all_slide_format_admin_panels_are_displayed_on_slide_admin_page() {
		...
		$slide_formats = ScreenWP_Slides::get_slide_formats();
		foreach ( $slide_formats as $key => $slide_format ) {
			$this->assertContains( '<div id="screenwp_slide_format_' . $key . '"', $meta_boxes );
		}
	}
	...
	function test_all_slide_background_admin_panels_are_displayed_on_slide_admin_page() {
		...
		$slide_backgrounds = ScreenWP_Slides::get_slide_backgrounds();
		foreach ( $slide_backgrounds as $key => $slide_background ) {
			$this->assertContains( '<div id="screenwp_slide_background_' . $key . '"', $meta_boxes );
		}
	}
	...
	function test_all_single_slide_options_are_displayed_for_their_optgroup() {
		...
		ob_start();
		ScreenWP_Admin_Slide::slide_format_options_html( new ScreenWP_Slide( $this->slide1 ), false );
		$actual = ob_get_clean();
	...
	function test_all_slide_stack_options_are_displayed_for_their_optgroup() {
		...
		ob_start();
		ScreenWP_Admin_Slide::slide_format_options_html( new ScreenWP_Slide( $this->slide1 ), true );
		$actual = ob_get_clean();
	...
	function test_slide_format_column_contains_format_and_background() {
		...
		ob_start();
		ScreenWP_Admin_Slide::do_slide_format_column( 'slide_format', $slide_id );
		$actual = ob_get_clean();
