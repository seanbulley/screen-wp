...
class Test_VUWU_Updater extends VUWU_UnitTestCase {
	...
	function test_is_database_version_updated_after_plugin_update() {
		...
		VUWU_Updater::update_db_version( '1.0.0' );

		$actual = VUWU_Updater::update();
		$expected = true;
		$this->assertEquals( $expected, $actual );

		$actual = VUWU_Updater::get_db_version();
		$expected = VUWU::get_version();
		$this->assertEquals( $expected, $actual );
	}
	...
	function test_is_database_version_updated_after_plugin_update_with_no_database_version_set() {
		...
		delete_option( 'vuwu_plugin_version' );
	...
	function test_is_database_update_skipped_when_database_is_up_to_date() {
		...
		VUWU_Updater::update_db_version( VUWU::get_version() );
	...
	function test_are_displays_reset_when_updating_from_1_0_0() {
		...
		$this->assertEmpty( get_post_meta( $this->display1, 'vuwu_reset_display', true ) );

		VUWU_Updater::update();

		/* Check that reset request was added */
		$this->assertNotEmpty( get_post_meta( $this->display1, 'vuwu_reset_display', true ) );
	}
