...
class VUWU_Admin_Channel {
	...
	static function add_slide_over_ajax() {
		...
		check_ajax_referer( 'vuwu_slides_editor_ajax_nonce', 'nonce' , true );
		...
		update_post_meta( $channel_id, VUWU_Slide::post_type_name, $new_slides );
	...
	static function get_set_duration_html( $post ) {
		...
		$default_duration = VUWU_Slides::get_default_slides_duration();

		$default_option_name = '(' . __( 'Default', 'vuwu' );
		if ( ! empty( $duration_options[ $default_duration ] ) ) {
			$default_option_name .= ' [' . $duration_options[ $default_duration ] . ']';
		}
		$default_option_name .= ')';

		$channel = new VUWU_Channel( $post );
		...
		?>
			<tr>
				<th>
					<label for="vuwu_slides_settings_duration">
						<?php echo esc_html__( 'Duration', 'vuwu' ); ?>
					</label>
				</th>
				<td>
					<select id="vuwu_slides_settings_duration" name="vuwu_slides_settings_duration">
	...
	static function get_set_transition_html( $post ) {
		...
		$default_transition = VUWU_Slides::get_default_slides_transition();
		...
		?>
			<tr>
				<th>
					<label for="vuwu_slides_settings_transition">
						<?php echo esc_html__( 'Transition', 'vuwu' ); ?>
					</label>
				</th>
				<td>
					<select id="vuwu_slides_settings_transition" name="vuwu_slides_settings_transition">
						<option value=""><?php echo esc_html( $default_option_name ); ?></option>
						<?php
	...
	static function get_slides_duration_options() {
		...
		foreach ( $secs as $sec ) {
			$slides_duration_options[ $sec ] = $sec . ' ' . _n( 'second', 'seconds', $sec, 'vuwu' );
		}
		 ...
		 */
		$slides_duration_options = apply_filters( 'vuwu/slides/duration/options', $slides_duration_options );
	 ...
	 * @since	1.0.1	Escaped and sanitized the output.
	 * @since	1.3.2	Changed method to static.
	 * @since	1.5.0	Added a vuwu-slide-is-stack class to stack slides.
	...
	static function get_slides_list_html( $post ) {
		 ...
		 *												admin screen or not.
		 */
		$display_slide_previews = apply_filters( 'vuwu/admin/channel/display_slide_previews', true );

		ob_start();

		?>
			<div class="vuwu_slides_editor_slides">
				<?php
					...
					if ( empty( $slides ) ) {
						...
						foreach( $slides as $slide ) {
							...
							$slide_url = add_query_arg( 'vuwu-preview', 1, $slide_url );
							$slide_format_data = VUWU_Slides::get_slide_format_by_slug( $slide->get_format() );
							$slide_background_data = VUWU_Slides::get_slide_background_by_slug( $slide->get_background() );

							?>
								<div class="vuwu_slides_editor_slides_slide<?php
									if ( $slide->is_stack() ) { echo ' vuwu-slide-is-stack'; }
								?>"
									data-slide-id="<?php echo intval( $slide->ID ); ?>"
									data-slide-key="<?php echo $i; ?>"
								>
									<div class="vuwu_slides_editor_slides_slide_iframe_container">
										<div class="vuwu_slides_editor_slides_slide_iframe_container_overlay">
											<h4><?php echo esc_html( get_the_title( $slide->ID ) ); ?></h4>
											<dl>
												<dt><?php _e( 'Format', 'vuwu'); ?></dt>
												<dd><?php echo esc_html( $slide_format_data['title'] ); ?></dd>
											</dl>
											<dl>
												<dt><?php _e( 'Background', 'vuwu'); ?></dt>
	...
	static function get_slides_transition_options() {
		 ...
		 */
		$slides_transition_options = apply_filters( 'vuwu/slides/transition/options', $slides_transition_options );
	...
	static function localize_scripts() {
                ...
                if ( empty( $screen ) || VUWU_Channel::post_type_name !== $screen->post_type ) {
                        return;
                }
                ...
                wp_localize_script( VUWU::get_plugin_name() . '-admin', 'vuwu_slides_editor_security', $security );
        }
	...
	static function save_channel( $post_id ) {
		...
		/* Check if our nonce is set */
                if ( ! isset( $_POST[VUWU_Channel::post_type_name.'_nonce'] ) ) {
                        return $post_id;
                }

                $nonce = sanitize_text_field( wp_unslash( $_POST[VUWU_Channel::post_type_name.'_nonce'] ) );

		/* Verify that the nonce is valid */
		if ( ! wp_verify_nonce( $nonce, VUWU_Channel::post_type_name ) ) {
			return $post_id;
		}
		...
		if (
			! isset( $_POST['vuwu_slides_settings_duration'] ) ||
			! isset( $_POST['vuwu_slides_settings_transition'] )
		) {
			return $post_id;
		}

                $vuwu_slides_settings_duration = intval( wp_unslash( $_POST['vuwu_slides_settings_duration'] ) );
                if ( empty( $vuwu_slides_settings_duration ) ) {
                        $vuwu_slides_settings_duration = '';
                }

                $vuwu_slides_settings_transition = sanitize_title( wp_unslash( $_POST['vuwu_slides_settings_transition'] ) );
		if ( empty( $vuwu_slides_settings_transition ) ) {
			$vuwu_slides_settings_transition = '';
		}

		update_post_meta( $post_id, VUWU_Channel::post_type_name . '_slides_duration' , $vuwu_slides_settings_duration );
		update_post_meta( $post_id, VUWU_Channel::post_type_name . '_slides_transition' , $vuwu_slides_settings_transition );
	}
	...
	static function slides_editor_meta_box( $post ) {
		...
		wp_nonce_field( VUWU_Channel::post_type_name, VUWU_Channel::post_type_name.'_nonce' );

		ob_start();

		?>
			<div class="vuwu_meta_box vuwu_slides_editor" data-channel-id="<?php echo intval( $post->ID ); ?>">
	...
	static function slides_settings_meta_box( $post ) {
		...
		?>
			<table class="vuwu_meta_box_form form-table vuwu_slides_settings_form">
				<tbody>
					<?php
