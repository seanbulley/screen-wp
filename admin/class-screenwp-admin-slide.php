 ...
 */
class ScreenWP_Admin_Slide {

	/**
	 * Adds the channel editor meta box to the display admin page.
	 *
	 * @since	1.0.0
	 * @since	1.3.1	Updated the slide_default_meta_box callback, after method was moved to ScreenWP_Admin_Slide_Format_Default.
	...
	static function add_slide_editor_meta_boxes() {
		add_meta_box(
			'screenwp_slide_content',
			__( 'Slide content' , 'screenwp' ),
			array( __CLASS__, 'slide_content_meta_box' ),
			ScreenWP_Slide::post_type_name,
			'normal',
			'low'
		);
	}
	...
	static function add_slide_format_column( $columns ) {
		...
		foreach( $columns as $key => $title ) {
			...
			if ( 'title' == $key ) {
				...
				$new_columns['slide_format'] = __( 'Slide format, background', 'screenwp' );
			}
		}
		return $new_columns;
	}
	...
	static function do_slide_format_column( $column, $post_id ) {
		if ( 'slide_format' == $column ) {
			...
			$slide = new ScreenWP_Slide( $post_id );

			$format = ScreenWP_Slides::get_slide_format_by_slug( $slide->get_format() );
			$background = ScreenWP_Slides::get_slide_background_by_slug( $slide->get_background() );
	...
	static function localize_scripts() {
		$slide_file_defaults = array(
			'image' => array(
				'text_select' => esc_html__( 'Select an image', 'screenwp' ),
				'text_use' => esc_html__( 'Use this image', 'screenwp' ),
			),
			'application/pdf' => array(
				'text_select' => esc_html__( 'Select a PDF', 'screenwp' ),
				'text_use' => esc_html__( 'Use this PDF', 'screenwp' ),
			),
			'video' => array(
				'text_select' => esc_html__( 'Select a video', 'screenwp' ),
				'text_use' => esc_html__( 'Use this video', 'screenwp' ),
			),
		);
		wp_localize_script( ScreenWP::get_plugin_name() . '-admin', 'screenwp_slide_file_defaults', $slide_file_defaults );

		$slide_formats_backgrounds = ScreenWP_Slides::get_slide_formats_backgrounds();
		wp_localize_script( ScreenWP::get_plugin_name() . '-admin', 'screenwp_slide_formats_backgrounds', $slide_formats_backgrounds );
	}
	...
	static function remove_sample_permalink( $sample_permalink ) {
		...
		if ( empty( $screen ) || ScreenWP_Slide::post_type_name != $screen->post_type ) {
	 ...
	 *
	 * @since	1.0.0
	 * @since	1.3.1	Updated the save_slide_default call, after method was moved to ScreenWP_Admin_Slide_Format_Default.
	 * @since	1.3.2	Changed method to static.
	...
	static function save_slide( $post_id ) {
		...
		if ( ! isset( $_POST[ScreenWP_Slide::post_type_name.'_nonce'] ) ) {
			return $post_id;
		}

		$nonce = $_POST[ScreenWP_Slide::post_type_name.'_nonce'];

		/* Verify that the nonce is valid */
		if ( ! wp_verify_nonce( $nonce, ScreenWP_Slide::post_type_name ) ) {
			return $post_id;
		}
		...
		$slide_format = ScreenWP_Slides::get_slide_format_by_slug( $slide_format_slug );

		/* Slide background */
		$slide_background_slug = sanitize_title( $_POST['slide_background'] );
		$slide_background = ScreenWP_Slides::get_slide_background_by_slug_for_slide_format( $slide_background_slug, $slide_format_slug );
	...
	static function slide_background_meta_box( $post ) {
		...
		wp_nonce_field( ScreenWP_Slide::post_type_name, ScreenWP_Slide::post_type_name.'_nonce' );

		$slide = new ScreenWP_Slide( $post->ID );

		?><input type="hidden" id="screenwp_slide_editor_<?php echo ScreenWP_Slide::post_type_name; ?>"
			name="screenwp_slide_editor_<?php echo ScreenWP_Slide::post_type_name; ?>" value="<?php echo intval( $post->ID ); ?>"><?php

		foreach( ScreenWP_Slides::get_slide_backgrounds() as $slide_background_key => $slide_background_data ) {
			?><label>
	...
	static function slide_content_meta_box( $post ) {
		...
		?><div class="screenwp_slide_select_format_background">

			<input type="hidden" id="screenwp_slide_editor_<?php echo ScreenWP_Slide::post_type_name; ?>"
				name="screenwp_slide_editor_<?php echo ScreenWP_Slide::post_type_name; ?>" value="<?php echo intval( $post->ID ); ?>">

			<div class="screenwp_slide_select_format">
				<p><?php _e( 'Format', 'screenwp' ); ?></p>
				<select name="slide_format">
					<optgroup label="<?php echo esc_attr( __( 'Single slides', 'screenwp' ) ); ?>">
						<?php self::slide_format_options_html( $slide, false ); ?>
					</optgroup>
					<optgroup label="<?php echo esc_attr( __( 'Magic slide stacks', 'screenwp' ) ); ?>">
						<?php self::slide_format_options_html( $slide, true ); ?>
					</optgroup>
				</select>
			</div>

			<div class="screenwp_slide_select_background">
				<p><?php _e( 'Background', 'screenwp' ); ?></p>
				<select name="slide_background">
					<?php foreach( ScreenWP_Slides::get_slide_backgrounds() as $slide_background_key => $slide_background_data ) { ?>
		...
		<div class="screenwp_slide_formats"><?php

			foreach( ScreenWP_Slides::get_slide_formats() as $slide_format_key => $slide_format_data ) {

				?><div id="<?php echo esc_attr( 'screenwp_slide_format_' . $slide_format_key ); ?>">
					<dl>
						<dt><?php echo esc_html( __( 'Format', 'screenwp') ); ?></dt>
						<dd><?php echo esc_html( $slide_format_data['title'] ); ?></dd>
					</dl>

					<?php if ( ! empty( $slide_format_data['description'] ) ) { ?>
						<p class="screenwp_slide_admin_description"><?php echo esc_html( $slide_format_data['description'] ); ?></p>
					<?php } ?>

					<?php if ( empty( $slide_format_data['description'] ) && empty( $slide_format_data['meta_box'] ) ) { ?>
						<p class="screenwp_slide_admin_description"><?php _e( 'No settings.', 'screenwp' ); ?></p>
					<?php } ?>
		...
		<div class="screenwp_slide_backgrounds"><?php

			foreach( ScreenWP_Slides::get_slide_backgrounds() as $slide_background_key => $slide_background_data ) {

				?><div id="<?php echo esc_attr( 'screenwp_slide_background_' . $slide_background_key ); ?>">
					<dl>
						<dt><?php echo esc_html( __( 'Background', 'screenwp') ); ?></dt>
						<dd><?php echo esc_html( $slide_background_data['title'] ); ?></dd>
					</dl>

					<?php if ( ! empty( $slide_background_data['description'] ) ) { ?>
						<p class="screenwp_slide_admin_description"><?php echo esc_html( $slide_background_data['description'] ); ?></p>
					<?php } ?>
	 ...
	 * @since	1.5.0
	 *
	 * @param	ScreenWP_Slide		$slide	The slide object of the current slide.
	...
	static function slide_format_options_html( $slide, $stack = false ) {
		...
		foreach( ScreenWP_Slides::get_slide_formats() as $slide_format_key => $slide_format_data ) {
			if ( $stack == ScreenWP_Slides::slide_format_is_stack( $slide_format_key ) ) {
