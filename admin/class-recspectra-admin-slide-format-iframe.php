 ...
 */
class Recspectra_Admin_Slide_Format_Iframe {
	...
	static function slide_meta_box( $post ) {
		...
		$placeholder = __( 'https://...', 'recspectra' );

		?><table class="form-table">
			<tbody>
				<tr>
					<th scope="row">
						<label for="slide_iframe_website_url"><?php _e( 'Web page URL', 'recspectra' ); ?></label>
					</th>
					<td>
						...
						<?php if ( $https ) { ?>
							<p><?php _e( 'Be sure to use an https URL', 'recspectra' ); ?></p>
						<?php } ?>
					</td>
				</tr>
			</tbody>
