 ...
 */
class Recspectra_Admin_Slide {

	/**
	 * Adds the channel editor meta box to the display admin page.
	 *
	 * @since	1.0.0
	 * @since	1.3.1	Updated the slide_default_meta_box callback, after method was moved to Recspectra_Admin_Slide_Format_Default.
	...
	static function add_slide_editor_meta_boxes() {
		add_meta_box(
			'recspectra_slide_content',
			__( 'Slide content' , 'recspectra' ),
	...
	static function add_slide_format_column( $columns ) {
		...
		foreach( $columns as $key => $title ) {
			...
			if ( 'title' == $key ) {
				...
				$new_columns['slide_format'] = __( 'Slide format, background', 'recspectra' );
			}
		}
		return $new_columns;
	}
	...
	static function do_slide_format_column( $column, $post_id ) {
		if ( 'slide_format' == $column ) {
			...
			$slide = new Recspectra_Slide( $post_id );

			$format = Recspectra_Slides::get_slide_format_by_slug( $slide->get_format() );
			$background = Recspectra_Slides::get_slide_background_by_slug( $slide->get_background() );
	...
	static function localize_scripts() {
		$slide_file_defaults = array(
			...
			'application/pdf' => array(
				...
				'text_use' => esc_html__( 'Use this PDF', 'recspectra' ),
			),
			'video' => array(
				'text_select' => esc_html__( 'Select a video', 'recspectra' ),
				'text_use' => esc_html__( 'Use this video', 'recspectra' ),
			),
		);
		wp_localize_script( Recspectra::get_plugin_name() . '-admin', 'recspectra_slide_file_defaults', $slide_file_defaults );

		$slide_formats_backgrounds = Recspectra_Slides::get_slide_formats_backgrounds();
		wp_localize_script( Recspectra::get_plugin_name() . '-admin', 'recspectra_slide_formats_backgrounds', $slide_formats_backgrounds );
	}
	 ...
	 *
	 * @since	1.0.0
	 * @since	1.3.1	Updated the save_slide_default call, after method was moved to Recspectra_Admin_Slide_Format_Default.
	 * @since	1.3.2	Changed method to static.
	...
	static function slide_background_meta_box( $post ) {
		...
		wp_nonce_field( Recspectra_Slide::post_type_name, Recspectra_Slide::post_type_name.'_nonce' );

		$slide = new Recspectra_Slide( $post->ID );

		?><input type="hidden" id="recspectra_slide_editor_<?php echo Recspectra_Slide::post_type_name; ?>"
			name="recspectra_slide_editor_<?php echo Recspectra_Slide::post_type_name; ?>" value="<?php echo intval( $post->ID ); ?>"><?php

		foreach( Recspectra_Slides::get_slide_backgrounds() as $slide_background_key => $slide_background_data ) {
			?><label>
	...
	static function slide_content_meta_box( $post ) {
		...
		?><div class="recspectra_slide_select_format_background">

			<input type="hidden" id="recspectra_slide_editor_<?php echo Recspectra_Slide::post_type_name; ?>"
				name="recspectra_slide_editor_<?php echo Recspectra_Slide::post_type_name; ?>" value="<?php echo intval( $post->ID ); ?>">

			<div class="recspectra_slide_select_format">
				<p><?php _e( 'Format', 'recspectra' ); ?></p>
				<select name="slide_format">
					<optgroup label="<?php echo esc_attr( __( 'Single slides', 'recspectra' ) ); ?>">
						<?php self::slide_format_options_html( $slide, false ); ?>
					</optgroup>
					<optgroup label="<?php echo esc_attr( __( 'Magic slide stacks', 'recspectra' ) ); ?>">
						<?php self::slide_format_options_html( $slide, true ); ?>
					</optgroup>
				</select>
			</div>

			<div class="recspectra_slide_select_background">
				<p><?php _e( 'Background', 'recspectra' ); ?></p>
				<select name="slide_background">
					<?php foreach( Recspectra_Slides::get_slide_backgrounds() as $slide_background_key => $slide_background_data ) { ?>
		...
		<div class="recspectra_slide_formats"><?php

			foreach( Recspectra_Slides::get_slide_formats() as $slide_format_key => $slide_format_data ) {

				?><div id="<?php echo esc_attr( 'recspectra_slide_format_' . $slide_format_key ); ?>">
					<dl>
						<dt><?php echo esc_html( __( 'Format', 'recspectra') ); ?></dt>
						<dd><?php echo esc_html( $slide_format_data['title'] ); ?></dd>
					</dl>

					<?php if ( ! empty( $slide_format_data['description'] ) ) { ?>
						<p class="recspectra_slide_admin_description"><?php echo esc_html( $slide_format_data['description'] ); ?></p>
					<?php } ?>

					<?php if ( empty( $slide_format_data['description'] ) && empty( $slide_format_data['meta_box'] ) ) { ?>
						<p class="recspectra_slide_admin_description"><?php _e( 'No settings.', 'recspectra' ); ?></p>
					<?php } ?>
		...
		<div class="recspectra_slide_backgrounds"><?php

			foreach( Recspectra_Slides::get_slide_backgrounds() as $slide_background_key => $slide_background_data ) {

				?><div id="<?php echo esc_attr( 'recspectra_slide_background_' . $slide_background_key ); ?>">
					<dl>
						<dt><?php echo esc_html( __( 'Background', 'recspectra') ); ?></dt>
						<dd><?php echo esc_html( $slide_background_data['title'] ); ?></dd>
					</dl>

					<?php if ( ! empty( $slide_background_data['description'] ) ) { ?>
						<p class="recspectra_slide_admin_description"><?php echo esc_html( $slide_background_data['description'] ); ?></p>
					<?php } ?>
	 ...
	 * @since	1.5.0
	 *
	 * @param	Recspectra_Slide		$slide	The slide object of the current slide.
	...
	static function slide_format_options_html( $slide, $stack = false ) {
		...
		foreach( Recspectra_Slides::get_slide_formats() as $slide_format_key => $slide_format_data ) {
			if ( $stack == Recspectra_Slides::slide_format_is_stack( $slide_format_key ) ) {
