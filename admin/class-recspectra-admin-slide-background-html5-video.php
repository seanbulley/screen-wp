 ...
 */
class Recspectra_Admin_Slide_Background_Html5_Video {
	...
	static function save_slide_background( $post_id ) {
                $slide_bg_html5_video_video = isset( $_POST['slide_bg_html5_video_video'] ) ? intval( wp_unslash( $_POST['slide_bg_html5_video_video'] ) ) : 0;
                if ( empty( $slide_bg_html5_video_video ) ) {
                        $slide_bg_html5_video_video = '';
                }

                $slide_bg_html5_video_video_url = '';
                if ( isset( $_POST['slide_bg_html5_video_video_url'] ) ) {
                        $slide_bg_html5_video_video_url = sanitize_text_field( wp_unslash( $_POST['slide_bg_html5_video_video_url'] ) );
                }

                $slide_bg_html5_video_video_start = '';
                if ( isset( $_POST['slide_bg_html5_video_video_start'] ) ) {
                        $slide_bg_html5_video_video_start = intval( wp_unslash( $_POST['slide_bg_html5_video_video_start'] ) );
                ...
                if ( isset( $_POST['slide_bg_html5_video_video_end'] ) ) {
                        $slide_bg_html5_video_video_end = intval( wp_unslash( $_POST['slide_bg_html5_video_video_end'] ) );
                ...
                if ( isset( $_POST['slide_bg_html5_video_hold_slide'] ) ) {
                        $slide_bg_html5_video_hold_slide = intval( wp_unslash( $_POST['slide_bg_html5_video_hold_slide'] ) );
                ...
                if ( isset( $_POST['slide_bg_html5_video_enable_sound'] ) ) {
                        $slide_bg_html5_video_enable_sound = intval( wp_unslash( $_POST['slide_bg_html5_video_enable_sound'] ) );
	...
	static function slide_background_meta_box( $post ) {
		...
		?><table class="form-table">
			<tbody>
				<tr>
					<th scope="row">
						<label for="slide_bg_html5_video_video_url"><?php _e( 'Video file', 'recspectra' ); ?></label>
					</th>
					<td>
						<div class="slide_file_field file_type_video<?php if ( empty( $slide_bg_image_image ) ) { ?> empty<?php } ?>" id="slide_bg_html5_video_file_field">
							...
							<p class="wp-ui-text-notification hidden" id="slide_bg_html5_video_video_url_notification">
								<?php printf( esc_html__( 'Unable to play video. Please check if the URL is valid, eg. %s', 'recspectra' ), 'https://archive.org/24/items/WildlifeSampleVideo/Wildlife.mp4' ); ?>
							</p>

							<input type="button" class="button slide_file_upload_button" value="<?php esc_html_e( 'Select video', 'recspectra' ); ?>" />
							<input type="button" class="button slide_file_delete_button" value="<?php esc_html_e( 'Remove video', 'recspectra' ); ?>" />
							...
							<p class="slide_file_empty_message"><?php _e( 'Select a video or manually enter a video URL.', 'recspectra' ); ?> <?php _e( 'For the best results use an MP4 video that is at least 1920 x 1080 pixels (landscape), or 1080 x 1920 pixels (portrait).', 'recspectra' ); ?></p>
						</div>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="slide_bg_html5_video_video_start"><?php _e('Start at', 'recspectra'); ?></label>
					</th>
					<td>
						...
						<span><?php _e( 'seconds', 'recspectra' ); ?></span>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="slide_bg_html5_video_video_end"><?php _e('End at', 'recspectra'); ?></label>
					</th>
					<td>
				...
				<tr>
					<th scope="row">
						<label for="slide_bg_html5_video_hold_slide"><?php _e('Hold slide until end?', 'recspectra'); ?></label>
					</th>
					<td>
						...
						<span><?php _e('Yes, hold the slide until the end of the video.', 'recspectra'); ?></span>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="slide_bg_html5_video_enable_sound"><?php _e('Enable sound?', 'recspectra'); ?></label>
					</th>
					<td>
						...
						<span><?php _e('Yes, enable sound for this video.', 'recspectra'); ?></span>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="slide_bg_html5_video_video_preview"><?php _e('Preview', 'recspectra'); ?></label>
					</th>
					<td>
