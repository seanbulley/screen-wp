...
class ScreenWP_Admin_Preview {
	...
	static function enqueue_scripts() {
		...
		wp_localize_script( ScreenWP::get_plugin_name() . '-admin', 'screenwp_preview', array(
		...
		if ( ! empty( $_GET['screenwp-preview'] ) ) {
			return;
		}
	...
	static function get_orientation_choice( $object_id ) {
		...
		$orientation_choices = get_user_meta( get_current_user_id( ), 'screenwp_preview_orientation_choices', true );
	...
	static function get_orientations() {
		...
		$orientations = array(
			'16-9' => __( 'Landscape', 'screenwp' ),
			'9-16' => __( 'Portrait', 'screenwp' ),
		);
	...
	static function save_orientation_choice( ) {
		...
		update_user_meta( get_current_user_id( ), 'screenwp_preview_orientation_choices', $orientation_choices );
