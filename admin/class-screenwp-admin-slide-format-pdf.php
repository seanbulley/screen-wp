 ...
 */
class ScreenWP_Admin_Slide_Format_PDF {

	/**
	 * Adds our own ScreenWP_Image_Editor_Imagick image editor to the list of available image editors.
	 *
	 * @since	1.1.0
	 *
	 * @param	array		The current list of image editors.
	 * @return	array		The list of image editors with our own ScreenWP_Image_Editor_Imagick added.
	 */
	static function add_screenwp_imagick_image_editor( $editors ) {
		...
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-screenwp-image-editor-imagick.php';

		$editors[] = 'ScreenWP_Image_Editor_Imagick';
		return $editors;
	}
	...
	static function add_pdf_images_to_attachment( $attachment_id ) {
		...
		$current_pdf_images = get_post_meta( $attachment_id, '_screenwp_pdf_images', true );
		...
		update_post_meta( $attachment_id, '_screenwp_pdf_images', $pdf_images );
	}
	...
	static function delete_pdf_images_for_attachment( $attachment_id ) {
		...
		$slide_images = get_post_meta( $attachment_id, '_screenwp_pdf_images', true );
	...
	static function display_admin_notice() {
		...
		if ( $error = get_transient( 'screenwp_save_slide_pdf_notice_' . get_the_ID() . '_' . get_current_user_id() ) ) { ?>
			<div class="notice notice-error">
				<p><?php _e( 'Processing PDF pages failed.', 'screenwp' ); ?></p>
				<p>
					<?php _e( 'Make sure you are on WordPress 4.7 or higher, and your webhosting has Imagick and Ghostscript installed.', 'screenwp' ); ?><br />
					<?php _e( 'Error message:', 'screenwp' ); ?> <?php echo $error->get_error_message(); ?>
				</p>
			</div><?php

			delete_transient( 'screenwp_save_slide_pdf_notice_' . get_the_ID() . '_' . get_current_user_id() );
		}
	}
	 ...
	 *
	 * Uses the ScreenWP_Image_Editor_Imagick image editor to convert PDF pages to PNG images.
	 *
	 * @since	1.1.0
	 * @since	1.3.1	Now invokes ScreenWP_Image_Editor_Imagick::pdf_setup() for WP < 4.7,
	 *					to make PDF processing work on WP < 4.7.
	 * @since	1.3.2	Returns a WP_Error for WP < 4.7, instead of invoking ScreenWP_Image_Editor_Imagick::pdf_setup()
	...
	static function generate_images_for_pdf_pages( $pdf_file ) {
		...
		if ( 'pdf' != $file_extension ) {
			return new WP_Error( 'invalid_image', __( 'Not a PDF file.', 'screenwp' ), $pdf_file );
		}

		$png_files = array();

		// Load our own ScreenWP_Image_Editor_Imagick by requesting methods that only exists in our image editor
		...
		if ( ! method_exists( 'WP_Image_Editor_Imagick', 'pdf_setup' ) ) {
			return new WP_Error(
				...
				__( 'Your WordPress version lacks support for PDF processing.', 'screenwp' ),
				$editor
			);
		}
	...
	static function save_slide_pdf( $post_id ) {
		...
		if ( empty( $slide_pdf_file ) ) {
			...
			if ( is_wp_error( $added ) ) {
				...
				set_transient( 'screenwp_save_slide_pdf_notice_' . $post_id . '_' . get_current_user_id(), $added, 45 );
	...
	static function slide_pdf_meta_box( $post ) {
		...
		?><table class="form-table">
			<tbody>
				<tr>
					<th scope="row">
						<label for="slide_pdf_file"><?php _e( 'PDF file', 'screenwp' ); ?></label>
					</th>
					<td>
						<div class="slide_file_field file_type_pdf<?php if ( empty( $slide_pdf_file ) ) { ?> empty<?php } ?>">
							...
							<input type="button" class="button slide_file_upload_button" value="<?php _e( 'Select PDF file', 'screenwp' ); ?>" />
							<input type="button" class="button slide_file_delete_button" value="<?php _e( 'Remove PDF file', 'screenwp' ); ?>" />
