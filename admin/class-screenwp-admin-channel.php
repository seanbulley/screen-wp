...
class ScreenWP_Admin_Channel {
	...
	static function add_slides_count_column( $columns ) {
		...
		foreach( $columns as $key => $title ) {
			...
			if ( 'title' == $key ) {
				...
				$new_columns['slides_count'] = __( 'Number of slides', 'screenwp' );
			}
		}
		return $new_columns;
	}
	...
	static function add_slide_over_ajax() {
		...
		check_ajax_referer( 'screenwp_slides_editor_ajax_nonce', 'nonce' , true );
		...
		update_post_meta( $channel_id, ScreenWP_Slide::post_type_name, $new_slides );
	...
	static function add_slides_editor_meta_box() {
		add_meta_box(
			'screenwp_slides_editor',
			_x( 'Slides', 'slide cpt', 'screenwp' ),
			array( __CLASS__, 'slides_editor_meta_box' ),
			ScreenWP_Channel::post_type_name,
			'normal',
			'high'
		);
	}
	...
	static function add_slides_settings_meta_box() {
		add_meta_box(
			'screenwp_slides_settings',
			__( 'Slideshow settings' , 'screenwp' ),
	...
	static function get_add_slide_html() {
		...
		?>
			<div class="screenwp_slides_editor_add">
				<table class="form-table">
					<tbody>
						<th>
							<label for="screenwp_slides_editor_add">
								<?php echo esc_html__( 'Add slide', 'screenwp' ); ?>
							</label>
						</th>
						<td>
							<select id="screenwp_slides_editor_add" class="screenwp_slides_editor_add_select">
								<option value="">(<?php echo esc_html__( 'Select a slide', 'screenwp' ); ?>)</option>
								<?php
									$slides = ScreenWP_Slides::get_posts();
									foreach ( $slides as $slide ) {
									?>
	...
	static function get_set_duration_html( $post ) {
		...
		$default_duration = ScreenWP_Slides::get_default_slides_duration();

		$default_option_name = '(' . __( 'Default', 'screenwp' );
		if ( ! empty( $duration_options[ $default_duration ] ) ) {
			$default_option_name .= ' [' . $duration_options[ $default_duration ] . ']';
		}
		$default_option_name .= ')';

		$channel = new ScreenWP_Channel( $post );
		...
		?>
			<tr>
				<th>
					<label for="screenwp_slides_settings_duration">
						<?php echo esc_html__( 'Duration', 'screenwp' ); ?>
					</label>
				</th>
				<td>
					<select id="screenwp_slides_settings_duration" name="screenwp_slides_settings_duration">
	...
	static function get_set_transition_html( $post ) {
		...
		$default_transition = ScreenWP_Slides::get_default_slides_transition();
		...
		?>
			<tr>
				<th>
					<label for="screenwp_slides_settings_transition">
						<?php echo esc_html__( 'Transition', 'screenwp' ); ?>
					</label>
				</th>
				<td>
					<select id="screenwp_slides_settings_transition" name="screenwp_slides_settings_transition">
						<option value=""><?php echo esc_html( $default_option_name ); ?></option>
						<?php
	...
	static function get_slides_duration_options() {
		...
		foreach ( $secs as $sec ) {
			$slides_duration_options[ $sec ] = $sec . ' ' . _n( 'second', 'seconds', $sec, 'screenwp' );
		}
		 ...
		 */
		$slides_duration_options = apply_filters( 'screenwp/slides/duration/options', $slides_duration_options );
	 ...
	 * @since	1.0.1	Escaped and sanitized the output.
	 * @since	1.3.2	Changed method to static.
	 * @since	1.5.0	Added a screenwp-slide-is-stack class to stack slides.
	...
	static function get_slides_list_html( $post ) {
		 ...
		 *												admin screen or not.
		 */
		$display_slide_previews = apply_filters( 'screenwp/admin/channel/display_slide_previews', true );

		ob_start();

		?>
			<div class="screenwp_slides_editor_slides">
				<?php

					if ( empty( $slides ) ) {
						?><p>
							<?php echo esc_html__( 'No slides in this channel yet.', 'screenwp' ); ?>
						</p><?php
					}
					else {
						...
						foreach( $slides as $slide ) {
							...
							$slide_url = add_query_arg( 'screenwp-preview', 1, $slide_url );
							$slide_format_data = ScreenWP_Slides::get_slide_format_by_slug( $slide->get_format() );
							$slide_background_data = ScreenWP_Slides::get_slide_background_by_slug( $slide->get_background() );

							?>
								<div class="screenwp_slides_editor_slides_slide<?php
									if ( $slide->is_stack() ) { echo ' screenwp-slide-is-stack'; }
								?>"
									data-slide-id="<?php echo intval( $slide->ID ); ?>"
									data-slide-key="<?php echo $i; ?>"
								>
									<div class="screenwp_slides_editor_slides_slide_iframe_container">
										<div class="screenwp_slides_editor_slides_slide_iframe_container_overlay">
											<h4><?php echo esc_html( get_the_title( $slide->ID ) ); ?></h4>
											<dl>
												<dt><?php _e( 'Format', 'screenwp'); ?></dt>
												<dd><?php echo esc_html( $slide_format_data['title'] ); ?></dd>
											</dl>
											<dl>
												<dt><?php _e( 'Background', 'screenwp'); ?></dt>
									...
									<div class="screenwp_slides_editor_slides_slide_caption">
										<?php echo esc_html_x( 'Slide', 'slide cpt', 'screenwp' ) . ' ' . ( $i + 1 ); ?>
										(<a href="#" class="screenwp_slides_editor_slides_slide_remove">x</a>)
									</div>
								</div>
							<?php
	...
	static function get_slides_transition_options() {
		...
		$slides_transition_options = array(
			'fade' => __( 'Fade', 'screenwp' ),
			'slide' => __( 'Slide', 'screenwp' ),
			'none' => __( 'No transition', 'screenwp' ),
		);
		 ...
		 */
		$slides_transition_options = apply_filters( 'screenwp/slides/transition/options', $slides_transition_options );
	...
	static function localize_scripts() {
		...
		$defaults = array( 'confirm_remove_message' => esc_html__( 'Are you sure you want to remove this slide from the channel?', 'screenwp' ) );
		wp_localize_script( ScreenWP::get_plugin_name() . '-admin', 'screenwp_slides_editor_defaults', $defaults );

		$security = array( 'nonce' => wp_create_nonce( 'screenwp_slides_editor_ajax_nonce' ) );
		wp_localize_script( ScreenWP::get_plugin_name() . '-admin', 'screenwp_slides_editor_security', $security );
	}
	...
	static function remove_sample_permalink( $sample_permalink ) {
		...
		if ( empty( $screen ) || ScreenWP_Channel::post_type_name != $screen->post_type ) {
	...
	static function save_channel( $post_id ) {
		...
		if ( ! isset( $_POST[ScreenWP_Channel::post_type_name.'_nonce'] ) ) {
			return $post_id;
		}

		$nonce = $_POST[ScreenWP_Channel::post_type_name.'_nonce'];

		/* Verify that the nonce is valid */
		if ( ! wp_verify_nonce( $nonce, ScreenWP_Channel::post_type_name ) ) {
			return $post_id;
		}
		...
		if (
			! isset( $_POST['screenwp_slides_settings_duration'] ) ||
			! isset( $_POST['screenwp_slides_settings_transition'] )
		) {
			return $post_id;
		}

		$screenwp_slides_settings_duration = intval( $_POST['screenwp_slides_settings_duration'] );
		if ( empty( $screenwp_slides_settings_duration ) ) {
			$screenwp_slides_settings_duration = '';
		}

		$screenwp_slides_settings_transition = sanitize_title( $_POST['screenwp_slides_settings_transition'] );
		if ( empty( $screenwp_slides_settings_transition ) ) {
			$screenwp_slides_settings_transition = '';
		}

		update_post_meta( $post_id, ScreenWP_Channel::post_type_name . '_slides_duration' , $screenwp_slides_settings_duration );
		update_post_meta( $post_id, ScreenWP_Channel::post_type_name . '_slides_transition' , $screenwp_slides_settings_transition );
	}
	...
	static function slides_editor_meta_box( $post ) {
		...
		wp_nonce_field( ScreenWP_Channel::post_type_name, ScreenWP_Channel::post_type_name.'_nonce' );

		ob_start();

		?>
			<div class="screenwp_meta_box screenwp_slides_editor" data-channel-id="<?php echo intval( $post->ID ); ?>">
	...
	static function slides_settings_meta_box( $post ) {
		...
		?>
			<table class="screenwp_meta_box_form form-table screenwp_slides_settings_form">
				<tbody>
					<?php
