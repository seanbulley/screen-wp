 ...
 */
class VUWU_Admin_Slide {

	/**
	 * Adds the channel editor meta box to the display admin page.
	 *
	 * @since	1.0.0
	 * @since	1.3.1	Updated the slide_default_meta_box callback, after method was moved to VUWU_Admin_Slide_Format_Default.
	...
	static function do_slide_format_column( $column, $post_id ) {
		if ( 'slide_format' == $column ) {
			...
			$slide = new VUWU_Slide( $post_id );

			$format = VUWU_Slides::get_slide_format_by_slug( $slide->get_format() );
			$background = VUWU_Slides::get_slide_background_by_slug( $slide->get_background() );
	...
	static function localize_scripts() {
                ...
                if ( empty( $screen ) || VUWU_Slide::post_type_name !== $screen->post_type ) {
                        return;
                }

                $slide_file_defaults = array(
                        'image' => array(
                                'text_select' => esc_html__( 'Select an image', 'vuwu' ),
                                'text_use' => esc_html__( 'Use this image', 'vuwu' ),
			),
			'application/pdf' => array(
				'text_select' => esc_html__( 'Select a PDF', 'vuwu' ),
				'text_use' => esc_html__( 'Use this PDF', 'vuwu' ),
			),
			'video' => array(
				'text_select' => esc_html__( 'Select a video', 'vuwu' ),
				'text_use' => esc_html__( 'Use this video', 'vuwu' ),
			),
		);
		wp_localize_script( VUWU::get_plugin_name() . '-admin', 'vuwu_slide_file_defaults', $slide_file_defaults );

		$slide_formats_backgrounds = VUWU_Slides::get_slide_formats_backgrounds();
		wp_localize_script( VUWU::get_plugin_name() . '-admin', 'vuwu_slide_formats_backgrounds', $slide_formats_backgrounds );
	}
	 ...
	 *
	 * @since	1.0.0
	 * @since	1.3.1	Updated the save_slide_default call, after method was moved to VUWU_Admin_Slide_Format_Default.
	 * @since	1.3.2	Changed method to static.
	...
	static function save_slide( $post_id ) {
		...
		/* Check if our nonce is set */
                if ( ! isset( $_POST[VUWU_Slide::post_type_name.'_nonce'] ) ) {
                        return $post_id;
                }

                $nonce = sanitize_text_field( wp_unslash( $_POST[VUWU_Slide::post_type_name.'_nonce'] ) );

		/* Verify that the nonce is valid */
		if ( ! wp_verify_nonce( $nonce, VUWU_Slide::post_type_name ) ) {
			return $post_id;
		}
		...
		$slide_format = VUWU_Slides::get_slide_format_by_slug( $slide_format_slug );

		/* Slide background */
                $slide_background_slug = sanitize_title( wp_unslash( $_POST['slide_background'] ) );
		$slide_background = VUWU_Slides::get_slide_background_by_slug_for_slide_format( $slide_background_slug, $slide_format_slug );
	...
	static function slide_background_meta_box( $post ) {
		...
		wp_nonce_field( VUWU_Slide::post_type_name, VUWU_Slide::post_type_name.'_nonce' );

		$slide = new VUWU_Slide( $post->ID );

		?><input type="hidden" id="vuwu_slide_editor_<?php echo VUWU_Slide::post_type_name; ?>"
			name="vuwu_slide_editor_<?php echo VUWU_Slide::post_type_name; ?>" value="<?php echo intval( $post->ID ); ?>"><?php

		foreach( VUWU_Slides::get_slide_backgrounds() as $slide_background_key => $slide_background_data ) {
			?><label>
	...
	static function slide_content_meta_box( $post ) {
		...
		?><div class="vuwu_slide_select_format_background">

			<input type="hidden" id="vuwu_slide_editor_<?php echo VUWU_Slide::post_type_name; ?>"
				name="vuwu_slide_editor_<?php echo VUWU_Slide::post_type_name; ?>" value="<?php echo intval( $post->ID ); ?>">

			<div class="vuwu_slide_select_format">
				<p><?php _e( 'Format', 'vuwu' ); ?></p>
				<select name="slide_format">
					<optgroup label="<?php echo esc_attr( __( 'Single slides', 'vuwu' ) ); ?>">
						<?php self::slide_format_options_html( $slide, false ); ?>
					</optgroup>
					<optgroup label="<?php echo esc_attr( __( 'Magic slide stacks', 'vuwu' ) ); ?>">
						<?php self::slide_format_options_html( $slide, true ); ?>
					</optgroup>
				</select>
			</div>

			<div class="vuwu_slide_select_background">
				<p><?php _e( 'Background', 'vuwu' ); ?></p>
				<select name="slide_background">
					<?php foreach( VUWU_Slides::get_slide_backgrounds() as $slide_background_key => $slide_background_data ) { ?>
		...
		<div class="vuwu_slide_formats"><?php

			foreach( VUWU_Slides::get_slide_formats() as $slide_format_key => $slide_format_data ) {

				?><div id="<?php echo esc_attr( 'vuwu_slide_format_' . $slide_format_key ); ?>">
					<dl>
						<dt><?php echo esc_html( __( 'Format', 'vuwu') ); ?></dt>
						<dd><?php echo esc_html( $slide_format_data['title'] ); ?></dd>
					</dl>

					<?php if ( ! empty( $slide_format_data['description'] ) ) { ?>
						<p class="vuwu_slide_admin_description"><?php echo esc_html( $slide_format_data['description'] ); ?></p>
					<?php } ?>

					<?php if ( empty( $slide_format_data['description'] ) && empty( $slide_format_data['meta_box'] ) ) { ?>
						<p class="vuwu_slide_admin_description"><?php _e( 'No settings.', 'vuwu' ); ?></p>
					<?php } ?>
		...
		<div class="vuwu_slide_backgrounds"><?php

			foreach( VUWU_Slides::get_slide_backgrounds() as $slide_background_key => $slide_background_data ) {

				?><div id="<?php echo esc_attr( 'vuwu_slide_background_' . $slide_background_key ); ?>">
					<dl>
						<dt><?php echo esc_html( __( 'Background', 'vuwu') ); ?></dt>
						<dd><?php echo esc_html( $slide_background_data['title'] ); ?></dd>
					</dl>

					<?php if ( ! empty( $slide_background_data['description'] ) ) { ?>
						<p class="vuwu_slide_admin_description"><?php echo esc_html( $slide_background_data['description'] ); ?></p>
					<?php } ?>
	 ...
	 * @since	1.5.0
	 *
	 * @param	VUWU_Slide		$slide	The slide object of the current slide.
	...
	static function slide_format_options_html( $slide, $stack = false ) {
		...
		foreach( VUWU_Slides::get_slide_formats() as $slide_format_key => $slide_format_data ) {
			if ( $stack == VUWU_Slides::slide_format_is_stack( $slide_format_key ) ) {
