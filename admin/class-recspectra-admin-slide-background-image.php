 ...
 */
class Recspectra_Admin_Slide_Background_Image {
	...
	static function slide_background_meta_box( $post ) {
		...
		?><table class="form-table">
			<tbody>
				<tr>
					<th scope="row">
						<label for="slide_bg_image_image"><?php esc_html_e( 'Background image', 'recspectra' ); ?></label>
					</th>
					<td>
						<div class="slide_file_field file_type_image<?php if ( empty( $slide_bg_image_image ) ) { ?> empty<?php } ?>">
							...
							<input type="button" class="button slide_file_upload_button" value="<?php esc_html_e( 'Select image', 'recspectra' ); ?>" />
							<input type="button" class="button slide_file_delete_button" value="<?php esc_html_e( 'Remove image', 'recspectra' ); ?>" />
							<input type="hidden" name="slide_bg_image_image" class="slide_file_value" value='<?php echo intval( $slide_bg_image_image ); ?>'>
							<p class="slide_file_empty_message"><?php _e( 'For the best results use an image that is at least 1920 x 1080 pixels (landscape), or 1080 x 1920 pixels (portrait).', 'recspectra' ); ?></p>
						</div>
					</td>
				</tr>
			</tbody>
