...
class ScreenWP_Admin_Display {
	...
	static function add_channel_columns($columns) {
		...
		return array_merge($columns,
			array(
				'default_channel' => __('Default channel', 'screenwp'),
				'active_channel' => __('Active channel', 'screenwp'),
			)
		);
	}
	...
	static function add_channel_editor_meta_box() {
		add_meta_box(
			'screenwp_channel_editor',
			_x( 'Channel', 'channel cpt', 'screenwp' ),
			array( __CLASS__, 'channel_editor_meta_box' ),
			ScreenWP_Display::post_type_name,
			'normal',
			'high'
		);
	}
	...
	static function add_channel_scheduler_meta_box() {
		add_meta_box(
			'screenwp_channel_scheduler',
			__( 'Schedule temporary channel' , 'screenwp' ),
	...
	static function channel_editor_meta_box( $post ) {
		...
		wp_nonce_field( ScreenWP_Display::post_type_name, ScreenWP_Display::post_type_name.'_nonce' );

		ob_start();

		?>
			<input type="hidden" id="screenwp_channel_editor_<?php echo ScreenWP_Display::post_type_name; ?>"
				name="screenwp_channel_editor_<?php echo ScreenWP_Display::post_type_name; ?>" value="<?php echo intval( $post->ID ); ?>">

			<table class="screenwp_meta_box_form form-table screenwp_channel_editor_form" data-display-id="<?php echo intval( $post->ID ); ?>">
				<tbody>
					<?php
	...
	static function do_channel_columns( $column, $post_id ) {
	    ...
	    switch ( $column ) {
		    ...
		    case 'active_channel' :
				...
				$display = new ScreenWP_Display( $post_id );
				...
				$channel = new ScreenWP_Channel( $active_channel_id );
		    ...
		    case 'default_channel' :
				...
				$channel = new ScreenWP_Channel( $default_channel_id );
	...
	static function get_channel_scheduler_defaults() {
		 ...
		 */
		return apply_filters( 'screenwp/channel_scheduler/defaults', $defaults );
	}
	...
	static function get_default_channel_html( $post ) {
		...
		$display = new ScreenWP_Display( $post );
		...
		?>
			<tr>
				<th>
					<label for="screenwp_channel_editor_default_channel">
						<?php echo esc_html__( 'Default channel', 'screenwp' ); ?>
					</label>
				</th>
				<td>
					<select id="screenwp_channel_editor_default_channel" name="screenwp_channel_editor_default_channel">
						<option value="">(<?php echo esc_html__( 'Select a channel', 'screenwp' ); ?>)</option>
						<?php
							$channels = ScreenWP_Channels::get_posts();
	...
	static function get_scheduled_channel_html( $post ) {
		...
		?>
			<tr>
				<th>
					<label for="screenwp_channel_editor_scheduled_channel">
						<?php echo esc_html__( 'Temporary channel', 'screenwp' ); ?>
					</label>
				</th>
				<td>
					<select id="screenwp_channel_editor_scheduled_channel" name="screenwp_channel_editor_scheduled_channel">
						<option value="">(<?php echo esc_html__( 'Select a channel', 'screenwp' ); ?>)</option>
						<?php
							$channels = ScreenWP_Channels::get_posts();
							foreach ( $channels as $channel ) {
			...
			<tr>
				<th>
					<label for="screenwp_channel_editor_scheduled_channel_start">
						<?php echo esc_html__( 'Show from', 'screenwp' ); ?>
					</label>
				</th>
				<td>
					<input type="text" id="screenwp_channel_editor_scheduled_channel_start" name="screenwp_channel_editor_scheduled_channel_start" value="<?php if ( ! empty( $scheduled_channel['start'] ) ) { echo esc_html( date_i18n( $channel_scheduler_defaults['datetime_format'], $scheduled_channel['start'] + get_option( 'gmt_offset' ) * HOUR_IN_SECONDS, true ) ); } ?>" />
				</td>
			</tr>
			<tr>
				<th>
					<label for="screenwp_channel_editor_scheduled_channel_end">
						<?php echo esc_html__( 'Until', 'screenwp' ); ?>
					</label>
				</th>
				<td>
					<input type="text" id="screenwp_channel_editor_scheduled_channel_end" name="screenwp_channel_editor_scheduled_channel_end" value="<?php if ( ! empty( $scheduled_channel['end'] ) ) { echo esc_html( date_i18n( $channel_scheduler_defaults['datetime_format'], $scheduled_channel['end'] + get_option( 'gmt_offset' ) * HOUR_IN_SECONDS, true ) ); } ?>" />
				</td>
			</tr>
		<?php
	...
	static function localize_scripts() {
		...
		$channel_scheduler_defaults = self::get_channel_scheduler_defaults();
		wp_localize_script( ScreenWP::get_plugin_name() . '-admin', 'screenwp_channel_scheduler_defaults', $channel_scheduler_defaults );
	}
	...
	static function save_display( $post_id ) {
		...
		if ( ! isset( $_POST[ScreenWP_Display::post_type_name.'_nonce'] ) ) {
			return $post_id;
		}

		$nonce = $_POST[ScreenWP_Display::post_type_name.'_nonce'];

		/* Verify that the nonce is valid */
		if ( ! wp_verify_nonce( $nonce, ScreenWP_Display::post_type_name ) ) {
			return $post_id;
		}
		...
		$channel = intval( $_POST['screenwp_channel_editor_default_channel'] );
		$display_id = intval( $_POST['screenwp_channel_editor_' . ScreenWP_Display::post_type_name] );
		...
		if ( ! empty( $channel ) ) {
			update_post_meta( $display_id, ScreenWP_Channel::post_type_name, $channel );
		}
		else {
			delete_post_meta( $display_id, ScreenWP_Channel::post_type_name );
		}
	...
	private static function save_schedule( $display_id ) {
		...
		delete_post_meta( $display_id, 'screenwp_display_schedule' );

		$screenwp_channel_editor_scheduled_channel = intval( $_POST['screenwp_channel_editor_scheduled_channel'] );
		if ( empty( $screenwp_channel_editor_scheduled_channel ) ) {
			return;
		}

		$screenwp_channel_editor_scheduled_channel_start = sanitize_text_field( $_POST['screenwp_channel_editor_scheduled_channel_start'] );
		if ( empty( $screenwp_channel_editor_scheduled_channel_start ) ) {
			return;
		}

		$screenwp_channel_editor_scheduled_channel_end = sanitize_text_field( $_POST['screenwp_channel_editor_scheduled_channel_end'] );
		if ( empty( $screenwp_channel_editor_scheduled_channel_end ) ) {
			return;
		}
		...
		$start = strtotime( $screenwp_channel_editor_scheduled_channel_start ) - get_option( 'gmt_offset' ) * HOUR_IN_SECONDS;
		$end = strtotime( $screenwp_channel_editor_scheduled_channel_end ) - get_option( 'gmt_offset' ) * HOUR_IN_SECONDS;
		...
		$schedule = array(
			'channel' => $screenwp_channel_editor_scheduled_channel,
			'start' => 	$start,
			'end' => $end,
		);

		add_post_meta( $display_id, 'screenwp_display_schedule', $schedule, false );
	}
}
